...
function renderDirectives(site, ind) {
  ...
  const locs = site.proxy.filter(p => LOCPATH_RE.test(String(p.path || '')))
  if (locs.length) {
    for (const loc of locs) lines.push(...renderLocation(loc, site, ind))
  } else {
...
function renderServers(site) {
  ...
  // ---- port 80 ----
  const b80 = ['server {', `    listen ${site.port};`, `    server_name ${domains};`]
  ...
  if (isHttps) {
    ...
    const b443 = [`    listen 443 ssl${site.listen.http2 ? ' http2' : ''};`]
    if (site.listen.http3) {
      b443.push('    listen 443 quic;', `    add_header Alt-Svc 'h3=":443"; ma=86400';`)
    }
    b443.unshift('server {')
