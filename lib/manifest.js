...
export function defaultSite(name) {
  return {
    ...
    proxy: [],            // { path: '/api', target: 'http://host:port' | 'https://host:port' | 'upstream:name',
                          //   verify?: true, ca?: '/abs/path.pem' } — TLS to the backend: check its certificate,
                          //   against `ca` or, when that is absent, the system trust store
...
// The one-level-deep keys. Also the list of real site fields that is not `defaultSite` itself —
// server.js's save-time key filter uses it as the second opinion on what a body may carry.
export const NESTED = ['https', 'hsts', 'listen', 'php', 'rateLimit', 'ipRules', 'basicAuth', 'gzip', 'staticCache']
...
const CA_PATH_RE = /^\/[^\s;{}"'\\]+$/
// Where Debian and Ubuntu keep the system trust store. An upstream holding a publicly-signed
// certificate has no CA file of its own to offer, and naming this one keeps the basis of a
// verification in the config rather than in whatever store a given nginx build falls back to.
...
export function validateSite(site) {
  ...
  for (const p of site.proxy) {
    ...
    // No CA file is not an error: without one the system trust store is used, which is what an
    // upstream with a public certificate needs. A path that *is* given still has to be a path.
    if (p.verify === true && p.ca && !CA_PATH_RE.test(String(p.ca))) {
      errs.push(`proxy ${p.path}: the CA file must be an absolute path (${p.ca})`)
    }
  }
...
function renderLocation(loc, site, ind) {
  ...
  const lines = [
    ...
    `${ind}    proxy_pass ${pass};`,
    // `$http_host` is the client's Host header verbatim, which is the form nginx documents for
    // passing it unchanged. `$host` is the same name with the port dropped, so a site on a
    // non-443 TLS port would have the port stripped before the backend ever saw it — and any
    // absolute URL the app builds from Host, a redirect most of all, would come back without it.
    `${ind}    proxy_set_header Host $http_host;`,
  ...
  // A pool is TLS if any of its backends is; a direct target is TLS if the scheme says so. The
  // verify flag lives on the rule and has to mean the same thing for both forms of target.
  const tlsUpstream = upstream ? isHttpsTarget(upstream) : /^https:\/\//i.test(String(loc.target || ''))
  if (tlsUpstream) {
    // SNI, unconditionally. It is off by default, and without it a backend that serves more than one
    // certificate over TLS has no way to know which one this request wants, so it answers with its
    // default server's — the same mechanism that makes virtual hosting over HTTPS work in the first
    // place. Harmless against a backend that only has one.
    lines.push(`${ind}    proxy_ssl_server_name on;`)
    // What the name is, when the default would be wrong. `proxy_ssl_name` defaults to the host part
    // of `proxy_pass`, which for a pool is the name this dashboard generated (`site_app`) — a name no
    // certificate carries. A direct https:// target already says the right thing. The name is also
    // the SNI, so a pool needs it whether or not verification is on.
    const sslName = upstream ? String(upstream.servers[0]?.host || '') : ''
    if (sslName) lines.push(`${ind}    proxy_ssl_name ${sslName};`)
    if (loc.verify === true) {
      // A trusted certificate is always named — the operator's CA when they gave one, the system
      // trust store otherwise — so what the check is against is visible in the config instead of
      // being whatever store this particular nginx build happens to fall back on. Depth 2 because a
      // public chain is a root plus an intermediate, and the default of 1 rejects the second.
      lines.push(`${ind}    proxy_ssl_verify on;`)
      lines.push(`${ind}    proxy_ssl_trusted_certificate ${toConf(String(loc.ca || SYSTEM_CA))};`)
      lines.push(`${ind}    proxy_ssl_verify_depth 2;`)
    } else {
