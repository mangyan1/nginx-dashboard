...
const CA_PATH_RE = /^\/[^\s;{}"'\\]+$/
// Where Debian and Ubuntu keep the system trust store. `proxy_ssl_verify on` without a
// `proxy_ssl_trusted_certificate` is a config error, so verification always needs *a* file to check
// against — and an upstream holding a publicly-signed certificate has no CA of its own to point at.
// This is that file, which is what makes "verify my upstream" expressible without one.
...
function renderLocation(loc, site, ind) {
  ...
  if (tlsUpstream) {
    if (loc.verify === true) {
      // A trusted certificate is always named, because `on` without one is a config error: the
      // operator's CA when they gave one, the system store otherwise. Depth 2 because a public chain
      // is a root plus an intermediate, and the default of 1 rejects the second.
