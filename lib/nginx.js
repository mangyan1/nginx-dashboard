 ...
 */
export async function safeApply(files, mutate, { reload = true, label = '', testLink } = {}) {
  ...
  if (reload) {
    ...
    // ponytail: exit 0 only proves the master was *signalled*, not that it adopted the config.
...
export async function revertHistory(id) {
  ...
  return safeApply(h.files.map(x => x.path), () => { for (const s of h.files) writeState(s) }, { label: `undo: ${h.label || 'change'}` })
}
