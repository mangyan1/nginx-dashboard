 ...
 * Backs up `files`, runs `mutate()` (awaited: the callers hash passwords with openssl inside the
 * transaction), tests with `nginx -t`; on failure restores
...
export async function safeApply(files, mutate, { reload = true, label = '', testLink, record = true } = {}) {
  ...
  try {
    // Awaited, not fired: an async mutate that is not awaited would still be writing when
    // nginx -t ran, and a throw in it would be an unhandled rejection instead of a rollback.
    await mutate()
  } catch (e) {
    restore(backups)
